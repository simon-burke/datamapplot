...
def _rgb_to_hex(rgb, round_to=30):
    """
    Converts a NumPy array of RGB/RGBA values to hex strings, optimized for rounded values.
    ...
    rgb = (30 * np.round(rgb / round_to)).astype(np.uint8)
...
def _interpolate_colors(tree, points, colors, target_points, nn=100):
    ...
    # Interpolate
    color_interpolator = RegularGridInterpolator((grid_x,grid_y), grid_colors, method="slinear")
...
def bundle_edges(data_map_coords, color_list, n_neighbors=10, color_map_nn=100, edges=None):
    """Use hammer edge bundling on nearest neighbors"""
    nbrs = NearestNeighbors(n_neighbors=max(n_neighbors, color_map_nn), algorithm="ball_tree", n_jobs=-1).fit(
        bundle_points
    )
    if edges is None:
    ...
    segment_colors = _interpolate_colors(nbrs, data_map_coords, color_list, midpoints, nn=color_map_nn)
